...
export const getPlaylistDetail = (id: number) => axios.get(`/playlist/detail?id=${id}`);
...
export const getPlaylistTrackAll = (id: number) => axios.get(`/playlist/track/all?id=${id}`);

// 歌手
// 获取歌手详情 /artist/detail?id=xx  id:歌手id
export const getArtistDetail = (id: number) => axios.get(`/artist/detail?id=${id}`);
