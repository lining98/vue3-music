		...
		<template #reference>
			<div>
				<ElInput
					...
					style="width: 250px"
				/>
				<el-button type="primary" :icon="Search" style="font-size: 20px" @click="search" />
			</div>
		</template>
		<div style="height: 480px">
			<el-scrollbar>
				<div v-if="showHot">
					...
					<ul class="list">
						<li v-for="(item, index) in searchHot" :key="item.searchWord" @click="hotClick(item.searchWord)">
							<div class="index">{{ index + 1 }}</div>
							<div class="text">
								<span class="name"
									>{{ item.searchWord }} <span class="content">{{ item.score }}</span></span
								>
