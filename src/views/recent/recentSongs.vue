		...
		<el-table :data="musicArr" style="width: 100%" :row-class-name="tableRowClassName" @row-dblclick="playSong">
			<el-table-column type="index" width="50">
				<template #default="scope, index">
					<IconPark v-if="scope.row.data.id === song.id" :icon="VolumeSmall" theme="filled" />
			...
			<el-table-column prop="data.al.name" label="专辑" min-width="260">
				<template #default="scope">
					<span class="clickable" @click="toAlbumDetails(scope.row.data.al.id)">{{ scope.row.data.al.name }}</span>
				</template>
			</el-table-column>
...
import { VolumeSmall } from '@icon-park/vue-next';
...
const getData = async () => {
	...
	musicArr.value.forEach((item: any, index: number) => (item.index = index + 1));
	loading.value = false;
};
