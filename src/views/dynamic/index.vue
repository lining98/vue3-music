<template>
	...
	>
					...
					<div class="item-content">
						...
						<div class="right">
							...
							<div class="song" v-if="JSON.parse(item.json).song">
								<div class="songimg" @click="play(JSON.parse(item.json).song?.id)">
		...
		<div class="loadMore">
			<el-button link type="primary" class="text-center w-full" :loading="loading">加载更多</el-button>
		</div>
	</div>
</template>
...
const getData = async () => {
	try {
		const { event, lasttime, more } = await getEvent(pagesize.value, lastTime.value);
		if (lastTime.value === -1) {
